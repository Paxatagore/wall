...
function __autoload($class_name) {
    ((@require_once "/home/asophie/www/deBrier/wall/inc/cl.".$class_name.".php") or (@require_once "/home/asophie/www/deBrier/wall/inc/cl.".$class_name.".php")) or die("Impossible de trouver ".$class_name) ;
}
...
function traitement($level = 1, $codeSQL="1 DAY") {
	$q = "SELECT DISTINCT message.*, personne.nom, personne.prenom FROM message, personne WHERE personne.num = message.auteur AND date BETWEEN DATE_SUB(NOW(), INTERVAL ".$codeSQL.") AND NOW() ORDER BY DATE ASC" ;
	...
	if ($req->rowCount() > 0) {
		...
		while ($row = $req->fetchObject()) {
			$message2 .= '<div><b>Le '.$row->date.', '.$row->prenom.' '.$row->nom.', a écrit le texte suivant :</b></div><div>'.$row->texte.'</div><hr>' ;
		}
		$p = new personne() ;
